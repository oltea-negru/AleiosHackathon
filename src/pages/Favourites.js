...
function Favourites()
{
    ...
    return (
        <div className='mt-5'>{dummyProducts.filter(product => product.favourite === true).map(product => <Product key={product.id} name={product.name} category={product.category} price={product.price} image={product.image} />)}</div>
    )
}
