...
function Tools()
{
    ...
    return (
        <div>{dummyProducts.filter(product => product.category === 'tools').map(product => <Product key={product.id} name={product.name} category={product.category} price={product.price} image={product.image} />)}</div>
    )
}
